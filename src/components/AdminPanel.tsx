...
export default function AdminPanel({ stage, onClose }: AdminPanelProps) {
  ...
  useEffect(() => {
    // re-render every 200ms so values stay in sync if game changes them
    ...
    <div className="admin-overlay" onClick={onClose}>
      <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="admin-actions">
          <button className="admin-btn" onClick={() => api.restart()}>
            ↻ 이 스테이지 다시 시작
          </button>
          <button className="admin-btn" onClick={copyHardcodedValues}>
            📋 현재 값 복사 (코드용)
          </button>
        </div>
          ...
          {api.tunables.fragments.map((f: any, idx: number) => (
            <div className="admin-frag" key={idx}>
              ...
              <div className="admin-row">
                ...
                <input
                  type="range"
                  min={api.fragmentSchema.xMin}
                  max={api.fragmentSchema.xMax}
                  step={10}
                  value={f.x}
                  onChange={(e) => handleFragmentChange(idx, 'x', parseFloat(e.target.value))}
                  className="admin-slider"
                />
              ...
              <div className="admin-row">
                ...
                <input
                  type="range"
                  min={api.fragmentSchema.yOffsetMin}
                  max={api.fragmentSchema.yOffsetMax}
                  step={1}
                  value={f.yOffset}
                  onChange={(e) =>
                    handleFragmentChange(idx, 'yOffset', parseFloat(e.target.value))
                  }
                  className="admin-slider"
                />
